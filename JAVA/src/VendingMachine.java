...
public class VendingMachine extends JFrame implements ActionListener
{
    ...
    JButton initChange = new JButton("", initChangeImage);


    JTextArea outputMessage = new JTextArea(2,30);

    JButton takeChange = new JButton("거스름돈");
    ...
    ImageIcon cfLabelicon = new ImageIcon(path+"images\\s4.jpg");
    ...
    ImageIcon CFfinishicon = new ImageIcon(path+"images\\s4.jpg");
    JButton CFfinished = new JButton("교환 완료", CFfinishicon);

    ImageIcon CFcloseicon = new ImageIcon(path+"images\\s3.jpg");
    JButton CFclose = new JButton("거래 종료", CFcloseicon);
...

    {
        ...
        for(int i = 0 ; i < Pstock.length ; i++){
            ...
            Image image = null;
            try {
                URL url = new URL((String) Pstock[i][3]);
            ...
            productPrice[i] = new JLabel(String.valueOf(Pstock[i][1])+"won", JLabel.CENTER);
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        for (int i = 0 ; i < nameB.size() ; i++){
            ...
            if(e.getSource() == bill[i]){
                ...
                //선택된 권액의 수량 재 설정
                totalInserted += Integer.parseInt(nameB.get(i).substring(0,nameB.get(i).indexOf("won")));;
        ...
        if (e.getSource() == enterCoins)
        {
            ...
            else if (totalInserted > productTotal)
            {
                ...
                //메시지 출력
                outputMessage.setText(wonFormat.format(productTotal) + " won만큼 사셨네요 감사합니다!\n거스름돈은 총 " + wonFormat.format(errorGiveChange) + "입니다!\n 거스름돈 반환 버튼을 눌러주세요.");
                outputMessage.setBackground(Color.green);
        ...
        for(int i = 0 ; i < nameB.size() ; i++){
            ...
            if (e.getSource() == change[i]) {
                ...
                calcChange -= Integer.parseInt(nameB.get(i).substring(0, nameB.get(i).indexOf("won")));
                CFchangeRemaining.setText(decFormat.format(calcChange));
